...
class COStencilTask_1: public tbb::task {
  private:
    LBM_GridPtr* toggle;
    int t0; int t1;
    int x0; int dx0;
  ...
  public:
    COStencilTask_1(LBM_GridPtr* toggle_,
                    int t0_, int t1_, 
    ...
    task* execute()
    {
        ...
        const int ds_x = 1;
        const int ds_y = 1;
        const int ds_z = 2; /* take the z data dependence in LBM_handleInOutFlow() into account */
            ...
            dt >= 1 && dx >= 2 * ds_x * dt * NPIECES)
        {
            ...
            {
                child = new(allocate_child()) COStencilTask_1(
                    toggle,
                    t0, t1,
...
class COStencilTask_2: public tbb::task {
  private:
    LBM_GridPtr* toggle;
    int t0; int t1;
    int x0; int dx0;
  ...
  public:
    COStencilTask_2(LBM_GridPtr* toggle_,
                    int t0_, int t1_, 
                    int x0_, int dx0_, int x1_, int dx1_,
                    int y0_, int dy0_, int y1_, int dy1_, 
                    int z0_, int dz0_, int z1_, int dz1_ ):
    ...
    task* execute()
    {
            ...
            dt >= 1 && dx >= 2 * ds_x * dt * NPIECES)
        {
            ...
            for (i = 0; i < NPIECES - 1; ++i)
            {
                child = new(allocate_child()) COStencilTask_2(
                    toggle,
                    t0, t1,
                    x0 + i * chunk, ds_x, x0 + (i+1) * chunk, -ds_x,
                    y0, dy0, y1, dy1,
...
int main( int nArgs, char* arg[] ) {
    ...
    {
        printf("Error: You need to set the environment var TBB_NUM_THREADS\n");
        exit(-1);
    }
    
    const int nthreads = atoi(tbb_num_threads);
    printf("Number of TBB threads used = %d\n", nthreads);
	...
	MAIN_startClock( &time );
        ...
        if ( param.simType == CHANNEL )
        {
            for (outer_t = 1; outer_t <= param.nTimeSteps; outer_t += SHOW_STATS_INTERVAL)
            {
                ...
                // Perform cache oblivious execution over SHOW_STATS_INTERVAL time steps                
                COStencilTask_1& a = *new(tbb::task::allocate_root())
                                     COStencilTask_1(toggleR,
                                                     t0, t1,
                                                     0, 0, SIZE_X, 0,
                                                     0, 0, SIZE_Y, 0,
                                                     0, 0, SIZE_Z, 0);
                ...
                if (t1 == (outer_t+SHOW_STATS_INTERVAL))
                {
                    printf( "timestep: %i\n", t1-1 );
        ...
        {  // don't call LBM_handleInOutFlow();
            ...
            for (outer_t = 1; outer_t <= param.nTimeSteps; outer_t += SHOW_STATS_INTERVAL)
            {
                ...
                // Perform cache-oblivious execution over SHOW_STATS_INTERVAL time steps
#if 1
                COStencilTask_2& a = *new(tbb::task::allocate_root())
                                     COStencilTask_2(toggleR,
                                                     t0, t1,
                                                     0, 0, SIZE_X, 0,
                                                     0, 0, SIZE_Y, 0,
                                                     0, 0, SIZE_Z, 0);
...
#ifdef VALIDATE
        co_basecase(toggleV, 1, param.nTimeSteps+1,
                    0, 0, SIZE_X, 0,
                    0, 0, SIZE_Y, 0,
                    0, 0, SIZE_Z, 0);
