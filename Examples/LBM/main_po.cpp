...
static PoCellEntry* poSrcGrid;
static PoCellEntry* poDstGrid;
...
void CopyLbmGridToPoGrid(POGRID poGrid, LBM_Grid lbmGrid)
{
    for (int z = 0; z < SIZE_Z; z++)
    {
        for (int y = 0; y < SIZE_Y; y++)            
        {
           for (int x = 0; x < SIZE_X; x++)            
           {
               ...
               PO_SRC_C(poGrid) = LOCAL(lbmGrid, C);
               PO_SRC_N(poGrid) = LOCAL(lbmGrid, N);
               PO_SRC_S(poGrid) = LOCAL(lbmGrid, S);
               PO_SRC_E(poGrid) = LOCAL(lbmGrid, E);
               PO_SRC_W(poGrid) = LOCAL(lbmGrid, W);
               PO_SRC_T(poGrid) = LOCAL(lbmGrid, T);
               PO_SRC_B(poGrid) = LOCAL(lbmGrid, B);
               PO_SRC_NE(poGrid) = LOCAL(lbmGrid, NE);
               PO_SRC_NW(poGrid) = LOCAL(lbmGrid, NW);
               PO_SRC_SE(poGrid) = LOCAL(lbmGrid, SE);
               PO_SRC_SW(poGrid) = LOCAL(lbmGrid, SW);
               PO_SRC_NT(poGrid) = LOCAL(lbmGrid, NT);
               PO_SRC_NB(poGrid) = LOCAL(lbmGrid, NB);
               PO_SRC_ST(poGrid) = LOCAL(lbmGrid, ST);
               PO_SRC_SB(poGrid) = LOCAL(lbmGrid, SB);
               PO_SRC_ET(poGrid) = LOCAL(lbmGrid, ET);
               PO_SRC_EB(poGrid) = LOCAL(lbmGrid, EB);
               PO_SRC_WT(poGrid) = LOCAL(lbmGrid, WT);
               PO_SRC_WB(poGrid) = LOCAL(lbmGrid, WB);
               PO_SRC_FLAGS(poGrid) = LOCAL(lbmGrid, FLAGS);               
           } 
        }
    }
}

void CopyPoGridToLbmGrid(POGRID poGrid, LBM_Grid lbmGrid)
{
    for (int z = 0; z < SIZE_Z; z++)
    {
        for (int y = 0; y < SIZE_Y; y++)            
        {
           for (int x = 0; x < SIZE_X; x++)            
           {
               ...
               LOCAL(lbmGrid, C) = PO_SRC_C(poGrid);
               LOCAL(lbmGrid, N) = PO_SRC_N(poGrid);
               LOCAL(lbmGrid, S) = PO_SRC_S(poGrid);
               LOCAL(lbmGrid, E) = PO_SRC_E(poGrid);
               LOCAL(lbmGrid, W) = PO_SRC_W(poGrid);
               LOCAL(lbmGrid, T) = PO_SRC_T(poGrid);               
               LOCAL(lbmGrid, B) = PO_SRC_B(poGrid);
               LOCAL(lbmGrid, NE) = PO_SRC_NE(poGrid);
               LOCAL(lbmGrid, NW) = PO_SRC_NW(poGrid);
               LOCAL(lbmGrid, SE) = PO_SRC_SE(poGrid);
               LOCAL(lbmGrid, SW) = PO_SRC_SW(poGrid);
               LOCAL(lbmGrid, NT) = PO_SRC_NT(poGrid);
               LOCAL(lbmGrid, NB) = PO_SRC_NB(poGrid);
               LOCAL(lbmGrid, ST) = PO_SRC_ST(poGrid);
               LOCAL(lbmGrid, SB) = PO_SRC_SB(poGrid);
               LOCAL(lbmGrid, ET) = PO_SRC_ET(poGrid);
               LOCAL(lbmGrid, EB) = PO_SRC_EB(poGrid);
               LOCAL(lbmGrid, WT) = PO_SRC_WT(poGrid);
               LOCAL(lbmGrid, WB) = PO_SRC_WB(poGrid);
               LOCAL(lbmGrid, FLAGS) = PO_SRC_FLAGS(poGrid);               
           } 
        }
    }
}
...
int main( int nArgs, char* arg[] ) {
    ...
    for( t = 1; t <= param.nTimeSteps; t++ ) {
        if( param.simType == CHANNEL ) {
            PO_handleInOutFlow(poSrcGrid);
        }
        
        PO_performStreamCollide(poSrcGrid, poDstGrid);
    ...
    // copy results to the lbm grids
    CopyPoGridToLbmGrid(poSrcGrid, *srcGrid);
    CopyPoGridToLbmGrid(poDstGrid, *dstGrid);
...
void MAIN_initialize( const MAIN_Param* param ) {
	...
	LBM_showGridStatistics( *srcGrid );
        ...
        // Initialize PoGrids from srcGrid and dstGrid
        poSrcGrid = new POGRID;
        assert(poSrcGrid);
        
        poDstGrid = new POGRID;
        assert(poDstGrid);

        // copy the initial lbm grids to the poGrids
        CopyLbmGridToPoGrid(poSrcGrid, *srcGrid);
        CopyLbmGridToPoGrid(poDstGrid, *dstGrid);
