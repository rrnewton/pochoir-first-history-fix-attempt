       ...
       if ( ( i >= 0 ) && ( i <= nX ) )
         {                      
           if ( ( i > 0 ) && ( j > 0 ) )
             {
#if DESUGAR
                if ( vD( t + 1, j ) < vG( t + 1, j ) + goCost ) vD( t + 2, j ) = vD( t + 1, j ) + geCost;
                else vD( t + 2, j ) = vG( t + 1, j ) + goCost + geCost;

                if ( vI( t + 1, j - 1 ) < vG( t + 1, j - 1 ) + goCost ) vI( t + 2, j ) = vI( t + 1, j - 1 ) + geCost;
                else vI( t + 2, j ) = vG( t + 1, j - 1 ) + goCost + geCost;
  
                if ( vD( t + 2, j ) < vI( t + 2, j ) ) v = vD( t + 2, j );
                else v = vI( t + 2, j );
                
                if ( X[ i ] == Y[ j ] ) k = 1;
                else k = 0;
                
                if ( v < vG( t, j - 1 ) + mmCost[ k ] ) vG( t + 2, j ) = v;
                else vG( t + 2, j ) = vG( t, j - 1 ) + mmCost[ k ]; 
#else
    ...
    if ( gotSeq )
      {
        ...
        int optCost = stencilPSA( nX, X, nY, Y, goCost, geCost, mmCost );    
        gettimeofday( &end, 0 );
