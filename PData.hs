...
instance Show Stmt where
    ...
    show (DEXPR declType expr) = show declType ++ " " ++ show expr ++ ";"
    ...
    show (CASE l_value tL@(t:ts)) = "case " ++ show l_value ++ " : " ++
        showList tL ""
    show (WHILE expr tL@(t:ts)) = "while " ++ show expr ++ "{" ++
        showList tL "" ++ breakline ++ "} /* end of while */" ++ breakline
...
getIterStmts :: [Stmt] -> Map.Map PName PArray -> [Iter]
getIterStmts [] _ = []
getIterStmts l_stmts@(a:as) l_arrayMap = 
    let i1 = getIterStmt a 
        i2 = getIterStmts as l_arrayMap
    in  union i1 i2
    where getIterStmt (BRACES stmts) = getIterStmts stmts l_arrayMap
          getIterStmt (EXPR e) = getIterExpr e
          getIterStmt (DEXPR t e) = getIterExpr e 
          getIterStmt (IF e s1 s2) = 
              let iter1 = getIterExpr e 
                  iter2 = getIterStmt s1 
                  iter3 = getIterStmt s2 
              in  union iter1 (union iter2 iter3)
          getIterStmt (SWITCH e stmts) = 
              let iter1 = getIterExpr e 
                  iter2 = getIterStmts stmts l_arrayMap
              in  union iter1 iter2
          getIterStmt (CASE v stmts) = getIterStmts stmts l_arrayMap
          getIterStmt (DEFAULT stmts) = getIterStmts stmts l_arrayMap
          getIterStmt NOP = []
          getIterStmt BREAK = []
          getIterStmt (DO e stmts) = 
              let iter1 = getIterExpr e 
                  iter2 = getIterStmts stmts l_arrayMap
              in  union iter1 iter2
          getIterStmt (WHILE e stmts) = 
              let iter1 = getIterExpr e 
                  iter2 = getIterStmts stmts l_arrayMap
              in  union iter1 iter2
          getIterStmt (FOR sL s) = 
              let iter1 = getIterStmt s 
                  iter2 = concat $ map (flip getIterStmts l_arrayMap) sL
              in  union iter1 iter2
          getIterStmt (UNKNOWN s) = []
          getIterExpr (VAR v) = []
          getIterExpr (PVAR v dL) = 
              case Map.lookup v l_arrayMap of
                   Nothing -> []
                   Just arrayInUse -> let iterName = "iter" 
                                          newIter = (iterName, arrayInUse, dL) 
                                      in  [newIter]
          getIterExpr (Uno uop e) = getIterExpr e
          getIterExpr (PostUno uop e) = getIterExpr e
          getIterExpr (Duo bop e1 e2) = 
              let iter1 = getIterExpr e1 
                  iter2 = getIterExpr e2
              in  (union iter1 iter2)
          getIterExpr (PARENS e) = getIterExpr e
          getIterExpr _ = []

transIterStmts :: [Stmt] -> [Iter] -> [Stmt]
transIterStmts [] _ = []
transIterStmts l_stmts@(a:as) l_iter = transIterStmt a : transIterStmts as l_iter
    where transIterStmt (BRACES stmts) = BRACES $ transIterStmts stmts l_iter
          transIterStmt (EXPR e) = EXPR $ transIterExpr e
          transIterStmt (DEXPR t e) = DEXPR t $ transIterExpr e 
          transIterStmt (IF e s1 s2) = IF (transIterExpr e) 
                                              (transIterStmt s1) 
                                              (transIterStmt s2) 
          transIterStmt (SWITCH e stmts) = SWITCH (transIterExpr e) 
                                                  (transIterStmts stmts l_iter)
          transIterStmt (CASE v stmts) = CASE v $ transIterStmts stmts l_iter
          transIterStmt (DEFAULT stmts) = DEFAULT $ transIterStmts stmts l_iter
          transIterStmt NOP =  NOP
          transIterStmt BREAK =  BREAK
          transIterStmt (DO e stmts) = DO (transIterExpr e) (transIterStmts stmts l_iter)
          transIterStmt (WHILE e stmts) = WHILE (transIterExpr e)
                                                (transIterStmts stmts l_iter)
          transIterStmt (FOR sL s) = FOR (map (flip transIterStmts l_iter) sL)
                                         (transIterStmt s) 
          transIterStmt (UNKNOWN s) = UNKNOWN s 
          transIterExpr (VAR v) =  VAR v
          transIterExpr (PVAR v dL) = 
              case pLookup (v, dL) l_iter of
                  Nothing -> PVAR v dL
                  Just iterName -> VAR iterName
          transIterExpr (Uno uop e) = Uno uop $ transIterExpr e
          transIterExpr (PostUno uop e) = PostUno uop $ transIterExpr e
          transIterExpr (Duo bop e1 e2) = Duo bop (transIterExpr e1) (transIterExpr e2)
          transIterExpr (PARENS e) = PARENS $ transIterExpr e
          transIterExpr (INT n) = (INT n)
          transIterExpr (FLOAT f) = (FLOAT f)
          transIterExpr (BOOL b) = (BOOL b)

pLookup :: (String, [DimExpr]) -> [Iter] -> Maybe String
pLookup (v, dL) [] = Nothing
pLookup (v, dL) ((iterName, arrayInUse, dim):is) 
    | v == aName arrayInUse && dL == dim = Just iterName
    | otherwise = pLookup (v, dL) is
...
pShowObaseKernel l_name l_kernel = 
    let l_rank = length (kParams l_kernel) - 1
        ...
        l_arrayName = unionArrayIter l_iter
    ...
    in  breakline ++ "Pochoir_obase_fn_" ++ show l_rank ++ 
        ...
        concat (intersperse ", " (map (getArrayGaps (l_rank-1)) l_arrayName)) ++ ";" ++
        breakline ++ pShowStrides l_rank l_arrayName ++ breakline ++
...
pShowObaseStmt l_kernel = 
    let oldStmts = kStmt l_kernel
        l_iter = kIter l_kernel
        obaseStmts = transIterStmts oldStmts l_iter
    in show obaseStmts
...
pShowIterSet iL@(i:is) l_kernelParams = concat $ map pShowIterSetTerm iL
    where pShowIterSetTerm (name, array, dim) = 
            breakline ++ name ++ ".set(" ++ pShowTransDim dim l_kernelParams ++ ");" 

pShowTransDim :: [DimExpr] -> [PName] -> String
pShowTransDim (d:ds) (p:ps) = 
    let l_rank = length ds
    in show (d:(pTransDim 1 l_rank ds ps))
...
pShowForHeader :: Int -> [String] -> [PName] -> String
...
adjustGap :: Int -> [String] -> String
adjustGap i aL@(a:as) = 
    if i > 0 then pShowAdjustGap i aL
             else ""
    where pShowAdjustGap i [] = ""
          pShowAdjustGap i aL@(a:as) = concat $ map (pShowAdjustGapTerm i) aL
          pShowAdjustGapTerm i a = "gap_" ++ a ++ "_" ++ show i ++ " = " ++
                                   "l_stride_" ++ a ++ "_" ++ show i ++ 
                                   " + (l_grid.x0[" ++ show (i-1) ++ "] - l_grid.x1[" ++
                                   show (i-1) ++ "]) * l_stride_" ++ a ++ "_" ++ show (i-1)
                                   ++ ";" ++ breakline
          
getIterName :: Iter -> String
getIterName (name, _, _) = name

getArrayGaps :: Int -> PName -> String
getArrayGaps 0 array = getArrayGap 0 array
getArrayGaps n array = getArrayGap n array ++ ", " ++ getArrayGaps (n-1) array
...
pShowArrayDynamicDecl :: [(PName, [Int])] -> String
...
pShowArrayItem :: (PName, [Int]) -> String
pShowArrayItem (a, bL@(b:bs)) = a ++ pShowArrayDim bL

pShowArrayDim :: [Int] -> String
pShowArrayDim [] = ""
